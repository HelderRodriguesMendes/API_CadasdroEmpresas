...
public interface CountryRepository extends JpaRepository<Country, Long> {
	...
	@Transactional
	@Query(value = "select * from country order by name limit 100", nativeQuery = true)
	Optional<List<Country>> findAllCountry();

	// PESQUISA COUNTRY POR NOME
	@Transactional
	@Query(value = "select * from country where name like %?1% order by name limit 100", nativeQuery = true)
